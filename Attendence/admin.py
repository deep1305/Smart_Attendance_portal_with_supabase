# Attendence/admin.py
from Attendence.utils import current_toronto_date
import streamlit as st
import pandas as pd
from github import GithubException
from .clients import create_supabase_client, create_github_repo
from .config import get_env
from .utils import current_toronto_date
...
def sidebar_controls(supabase):
    try:
        with st.sidebar:
            ...
            code_input = st.text_input("Class Code (For Students)", value="1234")
            if st.button("➕ Add Class"):
                if class_input.strip() and code_input.strip():
                    name_exists = supabase.table("classroom_settings").select("*").eq("class_name", class_input.strip()).execute().data
                    code_exists = supabase.table("classroom_settings").select("*").eq("code", code_input.strip()).execute().data
                    
                    if name_exists:
                        st.warning("A class with this name already exists.")
                    elif code_exists:
                        st.warning("This class code is already in use by another class. Please choose a unique code.")
                    else:
                        ...
                        st.success(f"Class '{class_input.strip()}' created with code '{code_input.strip()}'.")
                        st.rerun()
                elif class_input.strip():
                    st.warning("Please provide a class code.")
            ...
            strict_confirm = st.checkbox("I understand this is permanent and cannot be undone.")
            
            if st.button("🗑️ Delete This Class"):
                if delete_target.strip():
                    if confirm_delete != "DELETE":
                        st.warning("You must type exactly 'DELETE' in all caps to confirm.")
                    elif not strict_confirm:
                        st.warning("Please check the confirmation box to proceed.")
                    else:
                        exists = supabase.table("classroom_settings").select("*").eq("class_name", delete_target.strip()).execute().data
                        if not exists:
                            st.error(f"Class '{delete_target.strip()}' does not exist.")
                        else:
                            supabase.table("attendance").delete().eq("class_name", delete_target.strip()).execute()
                            supabase.table("roll_map").delete().eq("class_name", delete_target.strip()).execute()
                            supabase.table("classroom_settings").delete().eq("class_name", delete_target.strip()).execute()
                            st.success(f"Class '{delete_target.strip()}' permanently deleted.")
                            st.rerun()
                else:
                    st.warning("Please enter a class name to delete.")
...
def show_matrix_and_push(supabase, repo, selected_class):
    ...
    if records:
        ...
        if st.button("🚀 Push to GitHub"):
            ...
            filename = f"records/attendance_matrix_{selected_class}_{current_toronto_date().replace('-', '')}.csv"
